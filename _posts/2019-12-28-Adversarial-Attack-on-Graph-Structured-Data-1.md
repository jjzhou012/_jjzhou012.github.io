...
- 明确的语义。在这种情况下，一个黄金标准的分类器$f^{\ast}$可以被访问。等价指示器$\mathcal{I}(\cdot,\cdot,\cdot)$可以被定义为：

  $$
  \mathcal{I}(G, \tilde{G}, c)=\mathbb{I}\left(f^{*}(G, c)=f^{*}(\tilde{G}, c)\right)
  $$
  
  也就是说，对于节点$c$而言，修改前后的图都是它所在的图
...
上述公式中，$Q^{1 \ast}$和$Q^{2 \ast}$是实现$Q^{\ast}$的两个函数。只有当一对$(a_{t}^{(1)}, a_{t}^{(2)})$被选择时，一个action才算完成，所以只有当$a_{t}^{(2)}$完成时奖励才有效。这样的分解跟上述公式(6)有一样的优化结构，但是只需要$$O(x\times |V|)=O(|V|)$$的时间复杂度。
