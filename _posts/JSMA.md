...
&emsp;给定网络$F$，算法2通过扰动由saliency_map选择的 **两个输入特征（即，像素强度）$p_1$ 和 $p_2$ 来迭代地修改样本** 。 使用DNN的正向导数 $∇F(X^*)$ 在算法的每次迭代之间构建和更新显著图。 当满足以下条件之一时，该算法停止：
...
&emsp;其中 $ε(s，t，τ)$ 是相应成功率 $τ$ 的一组样本的平均失真。  
